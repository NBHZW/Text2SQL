...
// 结果汇总节点
@Component
class TestWrapUpNode(private val chatModel: ChatModel) : NodeAction {
    override fun apply(state: OverAllState): Map<String, Any> {
        val sceneLabel = state.value(TestGraphSpec.StateKey.SCENE_LABEL, "内容规划")
        // 上游节点虽然返回的是 ChatResponse 流，但 graph 在状态里保存的是“流结束后的最终消息”，
        // 所以下游这里可以直接按 AssistantMessage 读取，再拿 text 给下一个 prompt 使用。
        val draft = state.value(TestGraphSpec.StateKey.DRAFT, AssistantMessage::class.java).orElseThrow()
        val flux = ChatClient.create(chatModel)
            ...
            .stream()
            .chatResponse()
        return mapOf(TestGraphSpec.StateKey.FINAL_OUTPUT to flux)
    }
}