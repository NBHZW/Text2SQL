...
// 学习场景节点
@Component
class TestStudyPlanNode(private val chatModel: ChatModel) : NodeAction {
    override fun apply(state: OverAllState): Map<String, Any> {
        ...
        val flux = ChatClient.create(chatModel)
            ...
            .user("请根据这段需求给我一个简短学习计划：$input$feedbackPrompt")
            .options(OpenAiChatOptions.builder().extraBody(mapOf("enable_thinking" to false)).build())
            .stream()
            .chatResponse()
        return mapOf(TestGraphSpec.StateKey.DRAFT to flux)
    }
}
