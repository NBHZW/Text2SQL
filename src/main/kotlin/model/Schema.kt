...
data class Schema(
    ...
    private fun loadExamples(dbTable: DbTableSchemaView, dataSourceProvider: SchemaDataSourceProvider): Map<String, List<String>> {
        return runCatching {
    ...
    fun fetchDistinctValues(
        ...
        if (fullTableName.isNullOrBlank() || columnName.isNullOrBlank() || limit <= 0) {
            return emptyList()
        }
        val values: MutableList<String> = mutableListOf()
        val tableIdentifier = quoteSqliteIdentifier(fullTableName)
        val columnIdentifier = quoteSqliteIdentifier(columnName)
        // 使用 DISTINCT 确保去重，ORDER BY RANDOM() 随机采样避免数据倾斜
        val sql = String.format(
            "SELECT DISTINCT %s FROM %s WHERE %s IS NOT NULL ORDER BY RANDOM() LIMIT %d",
            columnIdentifier, tableIdentifier, columnIdentifier, limit
        )
        try {
